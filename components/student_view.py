...
def render_daily_plan(state):
    ...
    with col1:
        if st.button("Request Schedule Change"):
...
# At the beginning of the render_weekly_roadmap function
...
def render_weekly_roadmap(state):
    ...
    # Display weekly plan
    week_data = weeks_data[selected_week - 1] if selected_week is not None and selected_week <= len(weeks_data) else {}
